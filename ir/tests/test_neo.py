...
def test_fasttext():
    ...
    model = FastText([DEFAULT_ANALYZER(txtclean.lemmatize_raw_text(txtclean.preprocess_raw_text(doc))) for doc in DOCUMENTS], size=config.MODEL_SIZE, window=config.MODEL_WINDOW, min_count=config.MODEL_MIN_COUNT, workers=config.MODEL_WORKERS)
    # model.train(DOCUMENTS, total_examples=len(DOCUMENTS), epochs=config.MODEL_EPOCHS)

    # model = FastText([to_jaso(doc).split() for doc in DOCUMENTS], iter=3, min_count=1)
    # print([DEFAULT_ANALYZER(doc) for doc in DOCUMENTS])

    # model = FastText([doc.split() for doc in DOCUMENTS], iter=5, min_count=1)
    model.save('model_ft')
    print("save model_ft")
    # model.init_sims(replace=True)
