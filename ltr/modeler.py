...
class BaseRankModel(object):
    ...
    def _init_tf_vars(self):
        with tf.compat.v1.name_scope(self.model_name):
    ...
    def _score_fn_inner(self, x, reuse=False):
        ...
        # score
        score = tf.compat.v1.layers.dense(out, 1, activation=None,
                                kernel_initializer=tf.compat.v1.glorot_uniform_initializer(seed=self.params["random_seed"]))
    ...
    def _jacobian(self, y_flat, x):
        ...
        _, jacobian = tf.while_loop(
            cond=lambda j, _: j < self.batch_size,
            body=lambda j, result: (j + 1, result.write(j, tf.gradients(ys=y_flat[j], xs=x))),
            loop_vars=loop_vars)
    ...
    def _get_derivative(self, score, Wk, lambda_ij, x):
        ...
        shape = tf.concat(
            [tf.shape(input=lambda_ij), tf.ones([tf.rank(dsi_dWk_minus_dsj_dWk) - tf.rank(lambda_ij)], dtype=tf.int32)],
            axis=0)
        grad = tf.reduce_mean(input_tensor=tf.reshape(lambda_ij, shape) * dsi_dWk_minus_dsj_dWk, axis=[0, 1])
        return tf.reshape(grad, tf.shape(input=Wk))
    ...
    def save_session(self):
        ...
        tf.io.write_graph(self.sess.graph.as_graph_def(), self.params["offline_model_dir"], "graph.pb", as_text=True)
    ...
    def restore_session(self):
        ...
        self.saver.restore(self.sess, latest_dir + "model.checkpoint")
...
class DNN(BaseRankModel):
    ...
    def _build_model(self):
        ...
        loss = tf.reduce_mean(input_tensor=logloss)
        num_pairs = tf.shape(input=self.feature)[0]
...
class LogisticRegression(DNN):
    ...
    def _score_fn_inner(self, x, reuse=False):
        score = tf.compat.v1.layers.dense(x, 1, activation=None,
                                kernel_initializer=tf.compat.v1.glorot_uniform_initializer(seed=self.params["random_seed"]))
        return score
...
class RankNet(BaseRankModel):
    ...
    def _build_unfactorized_model(self):
        ...
        #
        S_ij = self.label - tf.transpose(a=self.label)
        S_ij = tf.maximum(tf.minimum(1., S_ij), -1.)
        P_ij = (1 / 2) * (1 + S_ij)
        s_i_minus_s_j = logits = score - tf.transpose(a=score)
        ...
        mask1 = tf.equal(self.qid - tf.transpose(a=self.qid), 0)
        mask1 = tf.cast(mask1, tf.float32)
        # exclude the pair of sample and itself
        n = tf.shape(input=self.feature)[0]
        mask2 = tf.ones([n, n]) - tf.linalg.tensor_diag(tf.ones([n]))
        mask = mask1 * mask2
        num_pairs = tf.reduce_sum(input_tensor=mask)

        loss = tf.cond(pred=tf.equal(num_pairs, 0), true_fn=lambda: 0., false_fn=lambda: tf.reduce_sum(input_tensor=logloss * mask) / num_pairs)
...
class LambdaRank(BaseRankModel):
    ...
    def _build_model(self):
        ...
        # current dcg
        index = tf.reshape(tf.range(1., tf.cast(self.batch_size, dtype=tf.float32) + 1), tf.shape(input=self.label))
        cg_discount = tf.math.log(1. + index)
        rel = 2 ** self.label - 1
        sorted_rel = 2 ** self.sorted_label - 1
        dcg_m = rel / cg_discount
        dcg = tf.reduce_sum(input_tensor=dcg_m)
        # every possible swapped dcg
        stale_ij = tf.tile(dcg_m, [1, self.batch_size])
        new_ij = rel / tf.transpose(a=cg_discount)
        stale_ji = tf.transpose(a=stale_ij)
        new_ji = tf.transpose(a=new_ij)
        # new dcg
        dcg_new = dcg - stale_ij + new_ij - stale_ji + new_ji
        # delta ndcg
        # sorted_label = tf.contrib.framework.sort(self.label, direction="DESCENDING")
        dcg_max = tf.reduce_sum(input_tensor=sorted_rel / cg_discount)
